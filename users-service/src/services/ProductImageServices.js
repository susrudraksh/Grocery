...
const ProductImagesServices = {
    createRecord: async(categoryData) => {
        return await ProductImages.create(categoryData).then(createRes => {
            return createRes;
        }).catch(err => {
            throw err;
        });
    ...
    allRecord: async (condition = {}) => {
        return await ProductImages.find(condition).then(createRes => {
            return createRes;
        }).catch(err => {
            throw err;
        });
    ...
    oneRecord: async (condition = {}) => {
        return await ProductImages.aggregate(condition).then(createRes => {
            return createRes;
        }).catch(err => {
            throw err;
        });
    ...
    updateRecord: async function (findPattern, updatePattern) {
        var options = { new: true };
         return await ProductImages.findOneAndUpdate(findPattern, updatePattern, options).then(updatedData => {
            return updatedData;
        }).catch(err => {
            throw err;
        });
    ...
    getPaginatedData: async function (findPattern, sortPattern, page_no=1, limit=10,select='') {
        ...
        return await ProductImages.paginate(query, options).then(async (paginatedData) => {
    ...
    deleteRecord: async function (deletePattern) {
        ...
        return ProductImages.findOneAndDelete(deletePattern)
            .then(deleteRes => {
                return deleteRes;
            }).catch(err => {
                throw err;
...
module.exports = ProductImagesServices