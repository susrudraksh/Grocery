...
class Sidebar extends Component {
  ...
  componentDidMount() {
    ...
    this.modifyMenuItemsByPermissions();
  }
    ...
    menuItems = menuItems.map((item) => {
      ...
      if (Array.isArray(item.module_slug)) {
        item.module_slug.map((item1) => {
          ...
          console.log(item1,hasPermission);
        });
      }
                  ...
                  return (
                    item.showItem && (
                      <Nav
                        key={item.id}
                        className={classnames({
                          "d-block": (this.state.selectedParentMenu === item.id && this.state.viewingParentMenu === "") || this.state.viewingParentMenu === item.id,
                        })}
                        data-parent={item.id}
                      >
                        {item.subs &&
