...
class DealOfDayList extends Component {
  constructor(props) {
    super(props);
    ...
    this.state = {
      ...
      addNewItemRoute: "/app/add-dealofday",
    ...
    swalWithBootstrapButtonsStatus
      ...
      .then(async (result) => {
        if (result.value) {
          ...
          let path = ApiRoutes.UPDATE_DEALOFDAY_STATUS + "/" + itemId;
  ...
  onDeleteItem = async (itemId, index) => {
    swalWithBootstrapButtonsDelete
      ...
      .then(async (result) => {
        if (result.value) {
          let path = ApiRoutes.DELETE_DEALOFDAY + "/" + itemId;
                      ...
                      {this.state.items.map((item, index) => {
                        return (
                          <tr key={index}>
                            ...
                            <td>
                              <img alt={item.title} src={item.image_thumb_url} className="img-thumbnail border-0 list-thumbnail align-self-center xsmall" />
                            </td>
                            <td>{item.title}</td>
                            ...
                            <td>
                              ...
                              <Link
                                to={{
                                  pathname: `edit-dealofday/${item._id}`,
                                  state: {
