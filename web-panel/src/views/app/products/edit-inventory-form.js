...
const InventoryFormSchema = Yup.object().shape({
  inventory_name: Yup.string().required("Please enter inventory name").min(2, "Too Short! Atleast 2 letters.").max(50, "Too Long! Atmost 50 letters."),
  hsn_code: Yup.string().required("Please enter hsn code").min(2, "Too Short! Atleast 2 letters.").max(50, "Too Long! Atmost 50 letters."),
  inventory_product_code: Yup.string().required("Please enter inventory product code").min(2, "Too Short! Atleast 2 letters.").max(50, "Too Long! Atmost 50 letters."),

  tax_rate: Yup.string().required("Please enter tax rate").matches(priceRegExp, "Invalid tax rate"),

  cgst_rate: Yup.string().when("tax_type", {
    is: (tax_type) => tax_type == "1",
    then: Yup.string().required("Please enter tax rate").matches(priceRegExp, "Invalid cgst  value"),
  }),
  min_inventory: Yup.string().required("Please enter minimum inventory").matches(priceRegExp, "Invalid minimum inventory value").max(5, "Too Long! Atmost 5 letters."),

  sgst_rate: Yup.string().when("tax_type", {
    is: (tax_type) => tax_type == "1",
    then: Yup.string().required("Please enter sgst rate").matches(priceRegExp, "Invalid sgst rate"),
  }),
  price: Yup.string().required("Please enter price").matches(priceRegExp, "Invalid price value").max(15, "Too Long! Atmost 15 letters."),
...
class InventoryForm extends Component {
  ...
  componentDidMount() {
    ...
    if (warehouseData.length > 0) {
      for (var i = 0; i < warehouseData.length; i++) {
        ...
        this.addCustomizeWarehouseForm(selectedId, warehouseTypeId, warhouseQuantity);
      }
    }
    ...
    if (type_id) {
      ...
      if (res) {
        if (res.status == 200) {
          ...
          prevCustomizeFormsList[index].customTypeValuesList = [...customTypeValuesList, ...customTypeValuesArr];
  ...
  addCustomizeWarehouseForm = (selectedId = "", selectedWarehouseType = "", selectedQuantityValue = "") => {
    this.setState((prevState) => ({
      customizeWarehouseForms: [
  ...
  onChangeWarehouseType = (value, index) => {
    let customizeWarehouseFormsClone = Object.assign([], this.state.customizeWarehouseForms);
  ...
  onRemoveCustomizeForm = async (index) => {
    ...
    if (customizeFormsClone.length > 2) {
      if (customizeFormsClone[index].id != "") {
        let path = ApiRoutes.DELETE_CUSTOMIZE_DATA + "/" + customizeFormsClone[index].id;
  ...
  onRemoveWarehouseCustomizeForm = async (index) => {
    ...
    if (customizeWarehouseFormsClone.length > 2) {
      if (customizeWarehouseFormsClone[index].id != "") {
        let path = ApiRoutes.DELETE_INVENTORY_DATA + "/" + this.state.inventoryId + "/" + customizeWarehouseFormsClone[index].id;
        const res = await Http("DELETE", path);
        if (res) {
          if (res.status == 200) {
      ...
      let message = "Please add a new record to delete this. There should atleast one record for an warehouse inventory.";
      NotificationManager.error(message, "Error!", 3000);
    }
  };
  ...
  handleFinalSubmit = async (e) => {
    ...
    setTimeout(async () => {
      ...
      formData.append("warehouse_inventory", JSON.stringify(customizeWarehouseFormsClone));
                ...
                <Colxx xxs="12" sm="12">
                  ...
                  >
                    {({ handleInventorySubmit, setFieldValue, setFieldTouched, handleChange, values, errors, touched, isSubmitting }) => (
                      <Form onChange={this.handleInventoryFormChange} className="av-tooltip tooltip-label-bottom">
                        <Row>
                          <Colxx xxs="12" sm="3">
                            <FormGroup className="form-group has-float-label">
                              <Label>Inventory Name</Label>
                              <Field className="form-control" name="inventory_name" type="text" />
                              {errors.inventory_name && touched.inventory_name ? <div className="invalid-feedback d-block">{errors.inventory_name}</div> : null}
                            </FormGroup>
                          </Colxx>
                          <Colxx xxs="12" sm="3">
                            <FormGroup className="form-group has-float-label">
                              <Label>HSN Code</Label>
                              <Field className="form-control" name="hsn_code" type="text" />
                              {errors.hsn_code && touched.hsn_code ? <div className="invalid-feedback d-block">{errors.hsn_code}</div> : null}
                            </FormGroup>
                          </Colxx>
                          <Colxx xxs="12" sm="3">
                            <FormGroup className="form-group has-float-label">
                              <Label>Inventory Product Code</Label>
                              <Field className="form-control" name="inventory_product_code" type="text" />
                              {errors.inventory_product_code && touched.inventory_product_code ? <div className="invalid-feedback d-block">{errors.inventory_product_code}</div> : null}
                            </FormGroup>
                          </Colxx>
                        </Row>
                        <Row>
                              ...
                              <Field
                                ...
                                onChange={(event) => {
                                  ...
                                  this.setState({ price: event.target.value }, () => {
                                    this.onChangeProductValue(this.state.price, setFieldValue);
                                  });
                                }}
                              />
                              {errors.price && touched.price ? <div className="invalid-feedback d-block">{errors.price}</div> : null}
                            </FormGroup>
                          </Colxx>
                          ...
                          <Colxx xxs="12" sm="3">
                            <FormGroup className="form-group has-float-label">
                              ...
                              <Field className="form-control" name="min_inventory" type="text" />
                              {errors.min_inventory && touched.min_inventory ? <div className="invalid-feedback d-block">{errors.min_inventory}</div> : null}
                            </FormGroup>
                          </Colxx>
                          ...
                          <Colxx xxs="12" sm="3">
                            <FormGroup className="error-l-175">
                              ...
                              <FormikCustomRadioGroup
                                ...
                                options={optionsType}
                              />
                              {errors.is_discount && touched.is_discount ? <div className="invalid-feedback d-block">{errors.is_discount}</div> : null}
                            </FormGroup>
                          </Colxx>
                        </Row>
                            ...
                            <Colxx xxs="12" sm="3">
                              <FormGroup className="form-group has-float-label">
                                ...
                                <select
                                  ...
                                  onChange={(event) => {
                                    setFieldValue("discount_type", event.target.value);
                                    this.setState({ discount_type: event.target.value }, () => {
                                      this.onChangeProductValue(this.state.discount_value, setFieldValue);
                                    });
                                  }}
                                >
                                  <option value="">Select</option>,<option value="1">Percentage (%)</option>,<option value="2">Fixed</option>
                                </select>
                                {errors.discount_type && touched.discount_type ? <div className="invalid-feedback d-block">{errors.discount_type}</div> : null}
                              </FormGroup>
                            </Colxx>
                            ...
                            <Colxx xxs="12" sm="3">
                              <FormGroup className="form-group has-float-label">
                                ...
                                <Field
                                  ...
                                  onChange={(event) => {
                                    setFieldValue("discount_value", event.target.value);
                                    this.setState({ discount_value: event.target.value }, () => {
                                      this.onChangeProductValue(this.state.discount_value, setFieldValue);
                                    });
                                  }}
                                />
                                {errors.discount_value && touched.discount_value ? <div className="invalid-feedback d-block">{errors.discount_value}</div> : null}
                              </FormGroup>
                            </Colxx>
                            <Colxx xxs="12" sm="3">
                              <FormGroup className="form-group has-float-label">
                                <Label>Discount Product Price</Label>
                                <Field className="form-control" name="discounted_product_price" type="text" readOnly value={values.discounted_product_price} />
                                {errors.discounted_product_price && touched.discounted_product_price ? <div className="invalid-feedback d-block">{errors.discounted_product_price}</div> : null}
                              </FormGroup>
                            </Colxx>
                          </Row>
                        )}
                        ...
                        <Row>
                          <Colxx xxs="12" sm="3">
                            <FormGroup className="error-l-175">
                              ...
                              />
                              {errors.tax_type && touched.tax_type ? <div className="invalid-feedback d-block">{errors.tax_type}</div> : null}
                            </FormGroup>
                          </Colxx>
                        </Row>
                              ...
                              <Field
                                ...
                                onChange={(event, value) => {
                                  ...
                                  this.setState({ tax_rate: event.target.value }, () => {
                                    this.onChangeProductValue(this.state.tax_rate, setFieldValue);
                                  });
                                }}
                              />
                              {errors.tax_rate && touched.tax_rate ? <div className="invalid-feedback d-block">{errors.tax_rate}</div> : null}
                            </FormGroup>
                          </Colxx>
                          ...
                          <Colxx xxs="12" sm="3">
                            <FormGroup className="form-group has-float-label">
                              ...
                              <Field
                                ...
                                type="text"
                                onChange={(event, value) => {
                                  setFieldValue("gst_amount", event.target.value);
                                  this.setState({ gst_amount: event.target.value }, () => {
                                    this.onChangeProductValue(this.state.gst_amount, setFieldValue);
                                  });
                                }}
                              />
                              {errors.gst_amount && touched.gst_amount ? <div className="invalid-feedback d-block">{errors.gst_amount}</div> : null}
                            </FormGroup>
                          </Colxx>
                          <Colxx xxs="12" sm="3">
                            <FormGroup className="form-group has-float-label">
                              ...
                              <Field
                                ...
                                type="text"
                                onChange={(event, value) => {
                                  setFieldValue("taxable_amount", event.target.value);
                                  this.setState({ taxable_amount: event.target.value }, () => {
                                    this.onChangeProductValue(this.state.taxable_amount, setFieldValue);
                                  });
                                }}
                              />
                              {errors.taxable_amount && touched.taxable_amount ? <div className="invalid-feedback d-block">{errors.taxable_amount}</div> : null}
                            </FormGroup>
                          </Colxx>
                        </Row>
                        {values.tax_type == "0" ? (
                            ...
                            <Colxx xxs="12" sm="3">
                              <FormGroup className="form-group has-float-label">
                                ...
                                <Field
                                  ...
                                  type="text"
                                  onChange={(event, value) => {
                                    setFieldValue("igst_rate", event.target.value);
                                    this.setState({ igst_rate: event.target.value }, () => {
                                      this.onChangeProductValue(this.state.igst_rate, setFieldValue);
                                    });
                                  }}
                                />
                                {errors.igst_rate && touched.igst_rate ? <div className="invalid-feedback d-block">{errors.igst_rate}</div> : null}
                              </FormGroup>
                            </Colxx>
                            <Colxx xxs="12" sm="3">
                              <FormGroup className="form-group has-float-label">
                                ...
                                <Field
                                  ...
                                  type="text"
                                  onChange={(event, value) => {
                                    setFieldValue("igst_amount", event.target.value);
                                    this.setState({ igst_amount: event.target.value }, () => {
                                      this.onChangeProductValue(this.state.igst_amount, setFieldValue);
                                    });
                                  }}
                                />
                                {errors.igst_amount && touched.igst_amount ? <div className="invalid-feedback d-block">{errors.igst_amount}</div> : null}
                              </FormGroup>
                            </Colxx>
                          </Row>
                        ) : (
                              ...
                              <Colxx xxs="12" sm="3">
                                <FormGroup className="form-group has-float-label">
                                  ...
                                  <Field
                                    ...
                                    type="text"
                                    onChange={(event, value) => {
                                      setFieldValue("cgst_rate", event.target.value);
                                      this.setState({ cgst_rate: event.target.value }, () => {
                                        this.onChangeProductValue(this.state.cgst_rate, setFieldValue);
                                      });
                                    }}
                                  />
                                  {errors.cgst_rate && touched.cgst_rate ? <div className="invalid-feedback d-block">{errors.cgst_rate}</div> : null}
                                </FormGroup>
                              </Colxx>
                              <Colxx xxs="12" sm="3">
                                <FormGroup className="form-group has-float-label">
                                  ...
                                  <Field
                                    ...
                                    readOnly
                                    onChange={(event, value) => {
                                      setFieldValue("cgst_amount", event.target.value);
                                      this.setState({ cgst_amount: event.target.value }, () => {
                                        this.onChangeProductValue(this.state.cgst_amount, setFieldValue);
                                      });
                                    }}
                                  />
                                  {errors.cgst_amount && touched.cgst_amount ? <div className="invalid-feedback d-block">{errors.cgst_amount}</div> : null}
                                </FormGroup>
                              </Colxx>
                            </Row>
                            <Row>
                              <Colxx xxs="12" sm="3">
                                <FormGroup className="form-group has-float-label">
                                  ...
                                  <Field
                                    ...
                                    type="text"
                                    onChange={(event, value) => {
                                      setFieldValue("sgst_rate", event.target.value);
                                      this.setState({ sgst_rate: event.target.value }, () => {
                                        this.onChangeProductValue(this.state.sgst_rate, setFieldValue);
                                      });
                                    }}
                                  />
                                  {errors.sgst_rate && touched.sgst_rate ? <div className="invalid-feedback d-block">{errors.sgst_rate}</div> : null}
                                </FormGroup>
                              </Colxx>
                              <Colxx xxs="12" sm="3">
                                <FormGroup className="form-group has-float-label">
                                  ...
                                  <Field
                                    ...
                                    readOnly
                                    onChange={(event, value) => {
                                      setFieldValue("sgst_amount", event.target.value);
                                      this.setState({ sgst_amount: event.target.value }, () => {
                                        this.onChangeProductValue(this.state.sgst_amount, setFieldValue);
                                      });
                                    }}
                                  />
                                  {errors.sgst_amount && touched.sgst_amount ? <div className="invalid-feedback d-block">{errors.sgst_amount}</div> : null}
                                </FormGroup>
                              </Colxx>
                            </Row>
                          </>
                        ...
                        <Row>
                          <Colxx xxs="12" sm="3">
                            <FormGroup className="form-group has-float-label">
                              ...
                              <Field className="form-control" name="sku_code" type="text" />
                              {errors.sku_code && touched.sku_code ? <div className="invalid-feedback d-block">{errors.sku_code}</div> : null}
                            </FormGroup>
                          </Colxx>
                          ...
                          <Colxx xxs="12" sm="3">
                            <FormGroup className="form-group has-float-label">
                              ...
                              <Field className="form-control" name="sku_name" type="text" />
                              {errors.sku_name && touched.sku_name ? <div className="invalid-feedback d-block">{errors.sku_name}</div> : null}
                            </FormGroup>
                          </Colxx>
                          ...
                          <Colxx xxs="12" sm="3">
                            <FormGroup className="form-group has-float-label">
                              <Label>Batch</Label>
                              <Field className="form-control" name="batch" type="text" />
                              {errors.batch && touched.batch ? <div className="invalid-feedback d-block">{errors.batch}</div> : null}
                            </FormGroup>
                          </Colxx>
                        </Row>

                        <Button className="hide-element" type="submit" innerRef={this.inventoryFormBtnRef}></Button>
                      </Form>
                    )}
                  </Formik>
                </Colxx>
                  ...
                  {this.state.customizeForms.map((value, index) => {
                    return (
                      <Formik
                        key={index}
                        initialValues={{
                          custom_type: this.state.customizeForms[index].custom_type,
                          custom_type_value: this.state.customizeForms[index].custom_type_value,
                        }}
                              ...
                              <Colxx xxs="12" sm="3">
                                <FormGroup className="form-group has-float-label">
                                  ...
                                  <select
                                    ...
                                    data-id={index}
                                    value={this.state.customizeForms[index].custom_type}
                                    onChange={(event) => {
                                      setFieldValue("custom_type", event.target.value);
                                      this.onChangeCustomType(event.target.value, index);
                                    }}
                                  >
                                    {this.state.customTypesList.map((item, index) => {
                                      return (
                                  ...
                                  {errors.custom_type && touched.custom_type ? <div className="invalid-feedback d-block">{errors.custom_type}</div> : null}
                                </FormGroup>
                              </Colxx>
                              <Colxx xxs="12" sm="3">
                                <FormGroup className="form-group has-float-label">
                                  ...
                                  <select
                                    ...
                                    data-id={index}
                                    value={this.state.customizeForms[index].custom_type_value}
                                    onChange={(event) => {
                                      setFieldValue("custom_type_value", event.target.value);
                                      this.onChangeCustomTypeValue(event.target.value, index);
                                    }}
                                  >
                                    {this.state.customizeFormsList[index].customTypeValuesList.map((item, index) => {
                                      return (
                                        <option key={index} value={item._id}>
                                          {item.name}
                                        </option>
                                  ...
                                  {errors.custom_type_value && touched.custom_type_value ? <div className="invalid-feedback d-block">{errors.custom_type_value}</div> : null}
                                </FormGroup>
                              </Colxx>
                              <Colxx xxs="12" sm="2">
                  ...
                  {this.state.customizeWarehouseForms.map((value, index) => {
                    return (
                      <Formik
                        key={index}
                        initialValues={{
                          warehouse_id: this.state.customizeWarehouseForms[index].warehouse_id,
                          quantity: this.state.customizeWarehouseForms[index].quantity,
                        }}
                        ...
                        {({ handleInventorySubmit, setFieldValue, setFieldTouched, handleChange, values, errors, touched, isSubmitting }) => (
                          ...
                          >
                            <Row>
                              <Colxx xxs="12" sm="3">
                                <FormGroup className="form-group has-float-label">
                                  ...
                                  <select
                                    ...
                                    data-id={index}
                                    value={this.state.customizeWarehouseForms[index].warehouse_id}
                                    onChange={(event) => {
                                      setFieldValue("warehouse_id", event.target.value);
                                      this.onChangeWarehouseType(event.target.value, index);
                                    }}
                                  >
                                ...
                                <FormGroup className="form-group has-float-label">
                                  ...
                                  <Field
                                    ...
                                    onChange={(event) => {
                                      ...
                                      this.onChangeQuantity(event.target.value, index);
                                    }}
                                  />
                              ...
                              <Colxx xxs="12" sm="2">
                                {this.state.customizeWarehouseForms[index + 1] ? (
                                  <Button
                                    outline
                                    color="danger"
                                    type="button"
                                    onClick={() => {
                                      this.onRemoveWarehouseCustomizeForm(index);
                                    }}
                                  >
                                    <div className="glyph-icon simple-icon-trash"></div>
                                  </Button>
