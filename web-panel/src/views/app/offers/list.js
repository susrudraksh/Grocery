...
class OfferList extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      addNewItemRoute: "/app/add-offers",

      pageSizes: [10, 20, 30],
      selectedPageSize: 10,
      dropdownSplitOpen: false,

      searchPlaceholder: "Search by Offer Title",
      searchKeyword: "",
      filterStatus: "",
    ...
    swalWithBootstrapButtonsStatus
      ...
      .then(async (result) => {
        if (result.value) {
          ...
          let path = ApiRoutes.UPDATE_OFFER_STATUS + "/" + itemId;
  ...
  onDeleteItem = async (itemId, index) => {
    swalWithBootstrapButtonsDelete
      ...
      .then(async (result) => {
        if (result.value) {
          let path = ApiRoutes.DELETE_OFFER + "/" + itemId;
                      ...
                      {this.state.items.map((item, index) => {
                        return (
                          <tr key={index}>
                            ...
                            <td>{item.endDate}</td>
                            {/* <td>
                              <NavLink to={`edit-brand/${item._id}`}>
                                {item.title}
                              </NavLink>
                            </td> */}
                            <td>
